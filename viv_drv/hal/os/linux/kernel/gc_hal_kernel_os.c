...
#ifdef CONFIG_G2D
extern struct clk 		*gclk_g2d;
#endif
#ifdef CONFIG_G3D
extern struct clk 		*gclk_g3d;
extern struct regulator *ggpu_vcc;
#endif
...
#define gcdUSE_NON_PAGED_MEMORY_CACHE 30 //10
    ...
    )
{
    ...
    else
    {
        ...
        {
            gcmkPRINT_N(0, "  DMA address is constant:\n");
            gcmkPRINT_N(4, "    0x%08X\n", dmaAddress1);
            gcmkPRINT_N(0, "  But state is changing:\n");
            gcmkPRINT_N(4, "    0x%08X\n", dmaState1);
            gcmkPRINT_N(4, "    0x%08X\n", dmaState2);
        }
        else
    ...
    )
{
    ...
    {
        gcmkPRINT("%s(%d) Id = %d is not found", __FUNCTION__, __LINE__, Id);
    ...
    )
{
    ...
    spin_lock_init(&integerDB.lock);

    /* Initialize signal id database. */
    idr_init(&integerDB.idr);
#if gcdUSE_NON_PAGED_MEMORY_CACHE
    os->cacheSize = 0;
...
{
    unsigned long flags;
    static struct timeval last_tv[2];
    ...
    if (Address == 0x10 || Address == 0xc)
    {
        struct timeval tv_temp;
        u32 diff;
        do_gettimeofday(&tv_temp);
        diff = (tv_temp.tv_sec - last_tv[Core].tv_sec) * USEC_PER_SEC
                    + tv_temp.tv_usec - last_tv[Core].tv_usec;
        // Delay 6us.
        if (diff <= 6) {
            //printk("((((( ReadRegisterEx[Core:%d] read [0x%x]: too near[%dus]!\n", Core, Address, diff);
            udelay(6 - diff);
        }
    }
    ...
    )
{
    ...
    writel_gpu_mutex(Data, (gctUINT8 *)Os->device->registerBases[Core] + Address);
    ...
    )
{
    ...
    case gcvBROADCAST_GPU_STUCK:
        printk("gcvBROADCAST_GPU_STUCK\n");
#if !gcdENABLE_RECOVERY
        gcmkONERROR(_DumpGPUState(Os, Hardware->core));
#endif
        /* using BUG_ON replace gpu recovery */
        gcmkONERROR(gckKERNEL_Recovery(Hardware->kernel));
        break;

    case gcvBROADCAST_AXI_BUS_ERROR:
        printk("gcvBROADCAST_AXI_BUS_ERROR\n");
#if !gcdENABLE_RECOVERY		
        gcmkONERROR(_DumpGPUState(Os, Hardware->core));
#endif
        /* using BUG_ON replace gpu recovery */
    ...
    )
{
    ...
    gctBOOL oldInterruptState = gcvFALSE;
    gctUINT32 control         = 0;
    ...
    if (Os->device->kernels[Core] != NULL)
    {
        ...
        oldInterruptState    = Os->device->kernels[Core]->hardware->interruptState;
    }
    
    if (!Power || !Clock)
    {
        if ((Os->device->kernels[Core] != NULL) && oldInterruptState) {
            //printk("!!!!!!!!! Disable int %d [Power=%d Clock=%d]\n", Core, Power, Clock);
            gckOS_SuspendInterruptEx(Os, Core);
            Os->device->kernels[Core]->hardware->interruptState = gcvFALSE;
            gckOS_ReadRegisterEx(Os, Core, 0x0, &control);
    ...
    else if (Core == gcvCORE_MAJOR)
    {
        ...
        {
            /*
            ret = regulator_set_voltage(pinfo->lcd_vcc1,1800000, 1800000);
            if (!ret) {
                printk(KERN_ERR, "GPU, %s: failed to set GPU to 1.8v \n", __FUNCTION__);
                goto err_regulator_set;   
            }
            */
            ret = regulator_enable(ggpu_vcc);
            if (ret) {
                printk(KERN_ERR "%s: failed to enable g3d regulator\n",__FUNCTION__);
            }    
        }
        else if (!Power && oldPowerState)
        {
            if(regulator_disable(ggpu_vcc))
    	    {
    ...
    if (Power && Clock)
    {
        if ((Os->device->kernels[Core] != NULL) && !oldInterruptState) {
            ...
            Os->device->kernels[Core]->hardware->interruptState = gcvTRUE;
        }
    }
...
{
    return _AllocateIntegerId(&integerDB, KernelPointer, Id);
}
...
{
    return _QueryIntegerId(&integerDB, Id, KernelPointer);
}
...
{
    return _DestroyIntegerId(&integerDB, Id);
}
