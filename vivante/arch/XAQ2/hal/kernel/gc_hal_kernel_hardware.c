    ...
    )
{
    ...
    hardware->interruptState  = gcvTRUE;
	...
	if (hardware->core == gcvCORE_2D) {
		hardware->powerOffTimeout = gcvPOWEROFF_TIMEOUT_2D;
	} else {
    ...
    )
{
    ...
    /* Delay 1us here to workaround 2D hang in S/R. DO NOT delete it until we find root cause. */
    udelay(1);
    ...
    )
{
    ...
    /*Last condition: Hardware->identity.chipRevision == 0x5220 && Hardware->identity.chipModel == gcv320*/
    if (1)
    {
        gctUINT32 debug;
    ...
    )
{
    ...
    /* Discard SUSPEND and IDLE when GPU is power off. */
    if ( (State == gcvPOWER_SUSPEND || State == gcvPOWER_IDLE)
    ...
    )
{
    ...
    if (Hardware->chipPowerState == gcvPOWER_IDLE)
    {
        *IsIdle = gcvTRUE;
    }
    else if (Hardware->chipPowerState == gcvPOWER_OFF)
    {
        //printk("Error!!!!!!!:%s, %d: chipPowerState == gcvPOWER_OFF\n", __FUNCTION__, __LINE__);
        *IsIdle = gcvFALSE;
    }
    else
    {
